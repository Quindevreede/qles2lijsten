...
public class GeheimeCodeQQ {
    ...
    public static void main(String[] secret) {
        ...
        System.out.println("Uncrypted List: ");
        printListWithIndex(uncryptList(secretAgent));
    ...
    private static String reReverseEncryptedName(String encryptedName) {
        String[] splitString = encryptedName.split("-");
    ...
    private static String uncryptName(String name) {
        StringBuilder uncryptedNameBuilder = new StringBuilder();
        for (int i = 0; i < name.length(); i++) {
            String number = charToNumber(name.charAt(i));
            uncryptedNameBuilder.append(number);
            if (i < name.length() - 1) {
                uncryptedNameBuilder.append("-");
            }
        }
        return uncryptedNameBuilder.toString();
    }

    private static String numberToChar(char ch) {
    ...
    private static List<String> uncryptList(List<String> members) {
        List<String> uncryptedList = new ArrayList<>();

        for (String member : members) {
            uncryptedList.add(uncryptName(member));
        }

        for (int i = 0; i < uncryptedList.size(); i++) {
            if (i % 2 == 0) {
                uncryptedList.set(i, reReverseEncryptedName(uncryptedList.get(i)));
            }
        }
        return uncryptedList;
    }
}